...
look at how to *sort*, *select*, *filter* and *mutate*. I will describe these
...
with dataframes, that is, how to perform operations on dataframes. In this
course, we will dive deep into complex operations but to begin with we will work
with the most basic ones.

### Working with Dataframes
There are a set of basic operations you should be familiar with to work on
dataframes. For instance, you might want to order the entries by one of the variables, e.g Age. This is called `sorting`.
You might not necessarily need all the variables of the data and only need a few
of them, choosing the variables you are interested in is called `selecting`.
You might only want one of the variables in the data to take
a single value, this is called `filtering`. Finally, you might need to add more
variables, or transform already existing data. This is called `mutating`. There
are many more operations we will learn, but we will begin these basic but powerful operations.
